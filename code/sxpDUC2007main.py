...
duc2007peersrcdir = r'E:\pythonworknew\code\textsum\data\DUC2007_Summarization_Documents\DUC2007_Summarization_Documents\summs\peers'

paperpath = './test/duc2007mult/papers'
fdir = './test/duc2007mult'
pkdir = r'./test/duc2007mult/papers_pk'
graph_dir =   './test/duc2007mult/papers_graph'
data_dir =r'./test/duc2007mult/data'
...
def main(cmdlist = [],summethod=[]):
    ...
    #    ,'loadalltopicsrc','makediffgraph','MakeSumByModel']
    # cmdlist=['PrepareTargetTopicSourceFile','loadalltopicsrc']
    #cmdlist = ['loadalltopicsrc']
    #cmdlist = ['makediffgraph','MakeSumByModel']
    ...
    if 'MakeSumByModel'in cmdlist:
        ...
        if 'worddirectedcloseone' in summethod:
            MakeSumByCloseOne('worddirectedcloseone')
...
def MakeSumByTFIDFOne(summethod = 'tfidfone'):
    ...
    sxpReadFileMan.SaveObject(model_sent_dict,fname)
def processmainsgml():
    soup = BeautifulSoup(open(duc2007sgml),features="lxml")
  ...
  #      print(topic.narr.get_text())
        ...
        print(doctopic)
        docs = topic.docs.get_text().strip()
        s = re.split('\s',docs)
        docidlist=[]
        for each in s:
            d= each.strip()
    ...
    fname = data_dir +'\\' + 'duc2007topictitle.dict'
...
def makemodelfortest(testname):
    ...
    for topicid, docdict in topic_dict.items():
        ...
        subdir = '\\D'+subdirid.groups()[0]
        srcmodelsubdir = duc2007modelsrcdir +'\\D'+subdirid.groups()[0]
        filenamelist = sxpReadFileMan.GetDirFileList(srcmodelsubdir)
...
def PrepareTargetTopicSourceFile():
    filelist, subdirlist = sxpReadFileMan.GetDir(duc2007srcdir)
    ...
    for eachsub in subdirlist:
       # print(eachsub)
        ...
        for eachf in filelist:
            ...
            sxptxt = sxpParseDUCText.ParseDUC2007(fname)
            rawdict = {}
...
def MakeDiffRank(summethod='diffsimgraph'):
    ...
    fname = pkdir + '\\' + 'sumby_'+summethod + '.dict'
    sxpReadFileMan.SaveObject(model_sent_dict,fname)
def MakeSumByClose(summethod='wordclosetopic'):
    print('build rank for',summethod)
    doc_model_sent_list_name =  graph_dir +'/'+'doc_model_sent_list.list'
    doc_model_sent_list=sxpReadFileMan.LoadObject(doc_model_sent_list_name)
    model_sent_dict = {}
    ...
    for doc_dict in doc_model_sent_list:
      ...
      #  keywordseq = [doc_dict['title']]
        keywordseq = [doc_dict['title'] + ' ' + doc_dict['narr']]
        if summethod == 'wordclosetopic':
            version = 'dual_v6'
        if summethod == 'worddirectedclose':
            version = 'dual_v6_directed'
        sent_rank_dict = sxpWordCloseQueryRank.queryrank(keywordseq,prefix, sentlist
                                                         ,wdlen = 250
                                                         ,simr = 0.5, removestop=True
                                                         ,version = version)
        wdlen = 0
        # sentlist = []
        # for s in ranksent:
    ...
    fname = pkdir + '\\' + 'sumby_'+summethod + '.dict'
    sxpReadFileMan.SaveObject(model_sent_dict,fname)
def MakeSumByCloseOne(summethod = 'wordclosetopicone'):
    print('build rank for',summethod)
    doc_model_sent_list_name =  graph_dir +'/'+'doc_model_sent_list.list'
    doc_model_sent_list=sxpReadFileMan.LoadObject(doc_model_sent_list_name)
    model_sent_dict = {}
    ...
    for doc_dict in doc_model_sent_list:
        ...
        for eachdoc in rawlist:
            thisdocsentlist = eachdoc['sxptxt'].sentence_list
            eachdocsent = []
            if summethod == 'wordclosetopicone':
                version = 'dual_v6'
            if summethod == 'worddirectedcloseone':
                version = 'dual_v6_directed'
            sent_rank_dict = sxpWordCloseQueryRank.queryrank(keywordseq, prefix, thisdocsentlist
                                                             , wdlen=250
                                                             ,simr = 0.4, removestop=True
...
def MakeSumBySimClose(summethod ='wordclosesim'):
    ...
    fname = pkdir + '\\' + 'sumby_'+summethod + '.dict'
    sxpReadFileMan.SaveObject(model_sent_dict,fname)
def MakeDUCPeers(summethod = 'duc2007s15'):
    g = re.match('duc2007s(\d+)',summethod)
    peerid = g.groups()[0]
    peerdir = r'E:\pythonworknew\code\textsum\data\DUC2007_Summarization_Documents\DUC2007_Summarization_Documents\summs\peers'
