...
def main():
    TestDemo()
   # RenameSystem()
def MakePaperPara(project_name,test_case_name,idname,model_test, lenstr='0'):
    # In this
  ...
  #  test_case_name= main_dir+'/' + test_case_name
    ...
    rank_para={
        ...
        'modelpattern':  r'#ID#.M.{0}.[A-Z].[A-Z].html'.format(lenstr), #'D061.M.010.J.B.html'
    ...
    return rank_para
def TestDemo():
    project_name = 'duc2002mult'
    test_case = 'mult100'
    idname ={'100':'01','top4':'02','manual100':'03'}
    model_idname = idname#sxpRougeConfig.idname
    model_test =['manual100']


     #this is to rank_parameter dict for this demo:
    lenstr = '100'
    rank_para = MakePaperPara(project_name, test_case, model_idname, model_test,lenstr)
...
##        doc_model_sent_file_list = sxpDataDucMultSum.LoadDocModelSentence()#[[['hellow'],['good']],[['doc2','sent1'], ['doc2', 'sent2']]]
##        DoMakeModel(project_name,test_case,doc_model_sent_file_list)
        PrepareModel(project_name,test_case)
    if 'rank' in cmd:
...
def PrepareModel(project_name,test_case_name):
    doc_model_sent_file_list=sxpDataDucMultSum.LoadDocModelSentence()
    fname_dict=GetFName(project_name,test_case_name)
  ...
  #  doc_sent_list = sxpReadFileMan.GetDirFile(raw_doc_dir,'pk')
    ...
    sxpReadFileMan.StoreSxptext(model_doc_list,fname_dict['model_doc_list'])
    return model_file_list
def ProduceTestRank(project_name,test_case,testset,lenstr='100'):
...
##            lens = s[1]
        ...
        #    graph_dict, matrix_dict,sentence_data_dict= sxpMultiPaperData.LoadFileData(graph_dict_fid)
            if eachtest != 'manual100':
                sxpdocdict = sxpDataDucMultSum.LoadRawPkByDocId(docid)
                tops = RankDocDemo(eachtest, sxpdocdict,docid,lenstr)
            else:
                tops = sxpDataDucMultSum.LoadManualSys(docid,lenstr='100',peeridx=0)
...
def RankDocDemo(eachtest,sxpdocdict,docid,lens='100'):
    tops = []
    if eachtest == 'abstract':
        for eachsent in GetAbsConSent(sxpdocdict.abstract):
            tops.append(eachsent)
    if eachtest == 'conclusion':
        for eachsent in GetAbsConSent(sxpdocdict.conclusion):
            tops.append(eachsent)
    if eachtest == 'top5':
         tops = GetTopK(sxpdocdict,docid,5)
    if eachtest == 'top4':
         tops = GetTopK(sxpdocdict,docid,4)
    if eachtest == 'random':
        tops = GetRandom(sxpdocdict,docid,4)
    if eachtest == '100':
        tops = GetLen(sxpdocdict,docid,lens)
    if eachtest == 'manual100':
        tops = sxpDataDucMultSum.LoadManualSys(docid,lenstr='100',peeridx=0)
    print(tops)
    return tops
...
def GetTopK(sxptxtlist,docid,topk):
    ...
    for i, s in enumerate( sxptxt.sentence_textset[0:topk]):
        tops.append(s)
    return tops
def GetLen(sxptxtlist,docid,lenstr='100'):
    sxplist=sxptxtlist[docid]
    sxptxtdict = sxplist[0]
    sxptxt =sxptxtdict['sxptxt']
    tops = []
    tl =0
    maxlen = int(lenstr)
    for i, s in enumerate( sxptxt.sentence_textset):
        ws = s.split(' ')
        l =0
        for eachw in ws:
