...
def MakePaperPara(project_name,test_case_name,idname,model_test):
  ...
  #  test_case_name= main_dir+'/' + test_case_name
    ...
    rank_para={
        ...
        'modelpattern':  r'#ID#.M.\d\d\d.[A-Z].[A-Z].html', #'D061.M.010.J.B.html'
...
def TestDemo():
    ...
    print('''
please NOTE that in this test, models with 010, 050, 100, and 200 lens are all used
for testing the generated sum which in manual100 are only of 100 words, so, the score
is not good as the test in sxpTestMultSumlen.py where model files are selected also
given by the predifined length. That, only using model with 0100 as the benchmark to 
compare.
''')
...
##            lens = s[1]
        ...
        #    graph_dict, matrix_dict,sentence_data_dict= sxpMultiPaperData.LoadFileData(graph_dict_fid)
            ...
            tops = RankDocDemo(eachtest, sxpdocdict,docid,lens)
...
def RankDocDemo(eachtest,sxpdocdict,docid,lens):
    tops = []
    ...
    if eachtest == 'manual100':
        tops = sxpDataDucMultSum.LoadManualSys(docid,'100',0)
    print(tops)
    return tops
